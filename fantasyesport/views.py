...
from fantasyesport.lib.battlecup import check_winner_exists, fight
from fantasyesport.lib.herolist_vals import heroes_init
from fantasyesport.util.random_function import add_months, bprint

from .models import (
    DBSession,
    User, Hero, TeamHeroLeague, Friend, Result, BattlecupUser, Battlecup)
...
def view_account(request):
    ...
    return HTTPFound("/viewLeague?league=1")
        ...
        if username:
            ...
            if user:
                if user.validate_password(request.params.get('password')):
                    ...
                    user.update({User.last_logged_in: datetime.datetime.now()})
                    return HTTPFound('/viewAccount', headers=headers)
                else:
