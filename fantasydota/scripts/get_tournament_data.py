    ...
    for match, series_id in matches:
        with transaction.manager:
            ...
            if add_match:
                ...
                try:
                    session.add(Match(
                        ...
                        match_json["radiant_win"], day, tournament_id, tournament_id
                    ))
                except:
                    print "Failed to add match: %s" % match_json["match_id"]
                    continue
            ...
            for key, value in enumerate(picks):
                ...
                # For if need to add results for calibration purposes. but dont want included in leagues
                session.add(Result(
                    tournament_id, value["hero_id"], int(match_json["match_id"]), result_string,
                    match_json["start_time"], series_id, (value["team"] == 0), match_json["start_time"]
                ))
    transaction.commit()
...
def main():
    ...
    tournaments = [9870]  # TI8 calibration tournaments
    for tournament in list(set(tournaments)):
        add_matches(session, tournament, tstamp_from=1534332914)
